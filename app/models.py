...
class User(Base):
    ...
    recommendations = relationship("StreamRecommendationAnalysis", back_populates="user")
...
class StreamRecommendationAnalysis(Base):
    __tablename__ = "stream_recommendations"
    
    id = Column(Integer, primary_key=True, index=True)
    user_id = Column(Integer, ForeignKey("users.id"))
    class_level = Column(String, nullable=True) # "10", "11", "12", "above_12"
    recommendation_type = Column(String) # "stream", "career", "goals"
    
    # For streams (10th going to 11th)
    stream_recommendation = Column(String, nullable=True) # e.g., "Science (PCM)"
    stream_reasoning = Column(Text, nullable=True)
    stream_pros = Column(JSON, nullable=True)
    stream_cons = Column(JSON, nullable=True)
    
    # For career guidance (11th students)
    career_suggestions = Column(JSON, nullable=True) # List of {"title": "", "reason": ""}
    career_reasoning = Column(Text, nullable=True)
    
    # For goals (12th and above)
    goal_suggestions = Column(JSON, nullable=True) # List of {"goal": "", "description": ""}
    goal_reasoning = Column(Text, nullable=True)
